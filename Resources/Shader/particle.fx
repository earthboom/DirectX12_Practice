...
VS_OUT VS_Main(VS_IN input)
{
	...
	worldPos += g_data[input.id].worldPos;	// id를 통해 찾음.
...
RWStructuredBuffer<Particle> g_particle : register(u0);	// 사실상 1차원 배열
RWStructuredBuffer<ComputeShared> g_shared : register(u1);	// 한개만 사용

// CS_Main
// g_vec2_1 : DeltaTime / AccTime;
// g_int 0 : Particle Max Count
// g_int_1 : AddCount;
...
void CS_Main(int3 threadIndex : SV_DispatchThreadID)
{
	...
	GroupMemoryBarrierWithGroupSync();	// 멀티 스레드 동기화 기법.

	// 0 (죽어있을 경우면)이면 생성하는 동작을 하고, 아니면 이동함.
	if (g_particle[threadIndex.x].alive == 0)
	{
		while (true)
		{
			...
			int originalValue;
			// 첫째, 둘째 매개변수가 같다면, 셋째값을 첫째값에 대입함.
			// 넷째값은 첫째, 둘째의 값 비교 결과에 상관없이 첫째값을 받음.
