...
{
    float4 Constant1110 = float4(1.0f, 1.0f, 1.0f, 0.0f);
...
{
    matrix MScaling = (matrix)0.0f;
    MScaling._11_22_33 = Scaling.xyz;
    float4 VRotationOrigin = float4(RotationOrigin.xyz, 0.0f);
    matrix MRotation = MatrixRotationQuaternion(RotationQuaternion);
    float4 VTranslation = float4(Translation.xyz, 0.0f);
...
float4 QuaternionSlerp(in float4 Q1, in float4 Q2, float Ratio)
{
    ...
    float4 SinOmega = float4(1.0f, 1.0f, 1.0f, 1.0f) - (CosOmega * CosOmega);
    ...
    float4 InvSinOmega = float4(1.0f, 1.0f, 1.0f, 1.0f) / SinOmega;
