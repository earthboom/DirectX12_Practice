...
void Shader::Init(const wstring& path, ShaderInfo info, const string& vs, const string& ps)
{
	...
	_pipelineDesc.DepthStencilState = CD3DX12_DEPTH_STENCIL_DESC(D3D12_DEFAULT);	//기본 Depth Stencil 설정
	_pipelineDesc.SampleMask = UINT_MAX;
	_pipelineDesc.PrimitiveTopologyType = info.topologyType;
	_pipelineDesc.NumRenderTargets = 1;
	_pipelineDesc.RTVFormats[0] = DXGI_FORMAT_R8G8B8A8_UNORM;
	_pipelineDesc.SampleDesc.Count = 1;
	_pipelineDesc.DSVFormat = DXGI_FORMAT_D32_FLOAT;//GEngine->GetDepthStencilBuffer()->GetDSVFormat();	//Depth Stencil View의 Format 설정
