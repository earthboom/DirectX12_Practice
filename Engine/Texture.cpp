...
void Texture::Load(const wstring& path)
{
	...
	_srvHandleBegin = _srvHeap->GetCPUDescriptorHandleForHeapStart();
	...
	DEVICE->CreateShaderResourceView(_tex2D.Get(), &srvDesc, _srvHandleBegin);
}

// Texture를 만드는데 필요한 정보를 모두 매개변수로 받는다
	...
	D3D12_RESOURCE_FLAGS resFlags, Vec4 clearColor)
{
	...
	//resFlags를 통해서 이 Teexture를 어떤 용도로 사용할 것인지 정함.
...
void Texture::CreateFromResource(ComPtr<ID3D12Resource> tex2D)
{
	...
	// 주요 조합
	// - DSV 단독 (조합X)
	// - SRV (일반적으로 이미지를 불러올 때)
	// - RTV + SRV (RTV로도 사용하고, RTV를 SRV처럼 이용 가능) 
	...
	else
	{
		...
		if (_desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS)
		{
			...
			_uavHandleBegin = _uavHeap->GetCPUDescriptorHandleForHeapStart();

			D3D12_UNORDERED_ACCESS_VIEW_DESC uavDesc = {};
			uavDesc.Format = _image.GetMetadata().format;
			uavDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;

			DEVICE->CreateUnorderedAccessView(_tex2D.Get(), nullptr, &uavDesc, _uavHandleBegin);
		}
