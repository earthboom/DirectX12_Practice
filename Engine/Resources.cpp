...
shared_ptr<Mesh> Resources::LoadPointMesh()
{
	...
	vector<Vertex> vec(1);
	vec[0] = Vertex(Vec3(0.0f, 0.0f, 0.0f), Vec2(0.5f, 0.5f), Vec3(0.0f, 0.0f, -1.0f), Vec3(1.0f, 0.0f, 0.0f));
...
shared_ptr<Mesh> Resources::LoadTerrainMesh(int32 sizeX, int32 sizeZ)
{
	...
	for (int32 z = 0; z < sizeZ; ++z)
	{
		for (int32 x = 0; x < sizeX; ++x)
		{
			...
			//   |	\
			//  [2] - [1]
			idx.emplace_back((sizeX + 1) * (z + 1) + (x));
			idx.emplace_back((sizeX + 1) * (z)+(x + 1));
			idx.emplace_back((sizeX + 1) * (z)+(x));
			...
			idx.emplace_back((sizeX + 1) * (z)+(x + 1));
			idx.emplace_back((sizeX + 1) * (z + 1) + (x));
			idx.emplace_back((sizeX + 1) * (z + 1) + (x + 1));
		}
	}
...
void Resources::CreateDefulatShader()
{
	//Skybox
	{
		ShaderInfo info =
